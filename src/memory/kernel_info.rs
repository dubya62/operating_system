...
use x86_64::{
    structures::paging::{
        frame::PhysFrame, mapper::MapToError, page::Page, FrameAllocator, Mapper, PageTableFlags,
        Size4KiB,
    },
    PhysAddr, VirtAddr,
};
...
pub fn init(
    ...
    FRAME_VIRTADDR.store(
        physical_memory_offset.as_u64() + frame.start_address().as_u64(),
        Ordering::Relaxed,
    );
    Ok(())
}
...
pub fn add_to_user_table(
    ...
    dbg!();
    let page = Page::containing_address(VirtAddr::new(KERNELINFO_VIRTADDR));
    let frame =
        PhysFrame::containing_address(PhysAddr::new(FRAME_PHYSADDR.load(Ordering::Relaxed)));
