...
pub mod AES{
    ...
    const RCON: [usize;11] = [0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];
    ...
    fn multiply(x: usize, y: usize) -> usize{
        return (((y & 1) * x) ^
           ...
           ((y>>4 & 1) * xtime(xtime(xtime(xtime(x)))))) & 0x255; /* this last call to xtime() can be omitted */
    }
    ...
    fn buffer_to_statet(buf: &mut [usize], i: usize) -> StateT{
        ...
        let result: [[usize;4];4] = [[buf[i+0], buf[i+1], buf[i+2], buf[i+3]], [buf[i+4], buf[i+5], buf[i+6], buf[i+7]], [buf[i+8], buf[i+9], buf[i+10], buf[i+11]], [buf[i+12], buf[i+13], buf[i+14], buf[i+15]]];
        return result;
    }
